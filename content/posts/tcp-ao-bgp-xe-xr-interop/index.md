...
- TCP-AO used for BGP session protection instead of TCP MD5.
...
- The key had to be entered as plaintext, but the syntax differed:
  ...
  - IOS-XR: `key-string clear <secret>`
- On IOS-XE, `include-tcp-options` had to be present both in the TCP key chain and under the BGP neighbor.
- On IOS-XR, `ao <keychain> include-tcp-options enable` under the BGP neighbor was sufficient.
